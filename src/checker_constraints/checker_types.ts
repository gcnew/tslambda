...
export {
    ...
    Type, Scheme, TypeEnv, Subst, Constraint, Unifier,

    InferState, TypeError,

    scheme, typeEnv, constr, unifier,
...
type Unifier = { kind: 'unifier', subst: Subst, constraints: List<Constraint> }
...
function constr(x: Type, y: Type): Constraint {
    return [x, y];
}

function unifier(subst: Subst, constraints: List<Constraint>): Unifier {
    return { kind: 'unifier', subst, constraints };
}
